...
'''def update_views(request):
    if request.method=='POST':
        c_id=int(request.POST.get('company_id'))

        if Careers_hub.objects.filter(id=c_id).exists():
            obj=Careers_hub.objects.get(id=c_id)

            obj.save()
            messages.info(request,'You are updated successfully..')
...
def delete_views(request):
    if request.method=='POST':
        ...
        if Careers_hub.objects.filter(id=pk).exists():
            obj=Careers_hub.objects.get(id=pk)
            obj.delete()
            messages.success(request,'You deleted successfully..')
            return redirect('career')
        else:
            messages.error(request,'Data not found to be delete...')
